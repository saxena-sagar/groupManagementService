...
class readThread extends Thread{
	...
	public void run() {
		...
		try {
			...
			MulticastSocket readSocket = new MulticastSocket();
			//making the socket join the multicast group
			readSocket.joinGroup(group);
			
			//keeping the listening thread always open and listening to the multicast port for any incoming message
			while(true) {
				
				byte[] incoming_message = new byte[MAX_MSG_LEN];
				DatagramPacket packet = new DatagramPacket(incoming_message,incoming_message.length,group,multcastPort);
...
public static void main(String[] args) throws Exception {
		...
		try {
			...
			//creating thread to listen to mulitple sources,if subscribed to multiple groups
			...
			//Here sending only one message so not running in loop but TODO need to substitute with sending heartbeat every 't' secs which would be in loop
			//sendSocket.send(packet);
